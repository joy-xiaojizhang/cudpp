...
__global__ void histogram_pre_scan_compaction(key_t* input, uint32_t* bin, uint32_t numElements, bucket_t bucket_identifier)
{
  ...
  if(blockIdx.x == (gridDim.x - 1)) // last block, potentially may try to read invalid inputs
  {
    ...
    for(int kk = 0; kk<DEPTH; kk++){
      ...
      uint32_t mask = __ballot_sync(WARP_MASK, valid_input);
#else
      ...
      // computing histogram
#if __CUDACC_VER_MAJOR__ >= 9
      uint32_t rx_buffer = __ballot_sync(WARP_MASK, bucket_identifier(myInput[kk]));
#else
...
#if __CUDACC_VER_MAJOR__ >= 9
      uint32_t rx_buffer = __ballot_sync(WARP_MASK, myBucket);
#else
...
#if __CUDACC_VER_MAJOR__ >= 9
      uint32_t n = __shfl_sync(scan_histo[kk], 0, 2);
#else
...
#if __CUDACC_VER_MAJOR__ >= 9
      myNewIndex[kk] += __shfl_sync(scan_histo[kk], myBucket, 32);
#else
...
#if __CUDACC_VER_MAJOR__ >= 9
      finalIndex -= __shfl_sync(scan_histo[kk], myNewBucket, 32);
#else
...
__global__ void multisplit_WMS_prescan(key_type* input, uint32_t* bin, uint32_t numElements, bucket_t bucket_identifier)
{
  ...
  if(blockIdx.x == (gridDim.x - 1)) // last block, potentially may try to read invalid inputs
  {
    ...
    for(int kk = 0; kk<DEPTH; kk++){
      ...
      {
#if __CUDACC_VER_MAJOR__ >= 9
        rx_buffer = __ballot_sync(WARP_MASK, bit & 0x01);
#else
...
__global__ void multisplit_WMS_postscan(key_type* key_input, uint32_t* warpOffsets, key_type* key_output, uint32_t numElements, bucket_t bucket_identifier)
{
  ...
  if(blockIdx.x == (gridDim.x-1))
  {
    ...
    for(int kk = 0; kk<DEPTH; kk++){
      ...
      {
#if __CUDACC_VER_MAJOR__ >= 9
        n = __shfl_up_sync(scan_histo[kk], i, 32);
#else
  ...
  if(blockIdx.x == (gridDim.x - 1)) // last block
  {
    ...
    for(int kk = 0; kk<DEPTH; kk++){
...
#if __CUDACC_VER_MAJOR__ >= 9
      uint32_t myLocalBlockIndex = __shfl_sync(scan_temp[kk], myBucket, 32);
#else
      ...
      if(warpId == (NUM_W-1)) // the last warp
      {
        ...
        {
#if __CUDACC_VER_MAJOR__ >= 9
          n = __shfl_up_sync(block_scan, i, 32);
#else
